...
for N in 10 25 50 100 200; do
	...
	for METHOD in svdquartets_paup; do
		...
		~/phylogenetic_tools/tree_comp/compareTrees.missingBranch ~/scratch/11-taxon_redo/${MODEL}/${SPECIESNUMBER}/S_relabeled_tree.trees ~/AGBsvdquartets/11-taxon_redo/11-taxon_svdquartets_paup_trees/${MODEL}/${METHOD}_R${SPECIESNUMBER}_${N}_sites_${K}_genes.tree >> ${MODELPATH}/temp_${METHOD}_${N}_${K}
	    done
